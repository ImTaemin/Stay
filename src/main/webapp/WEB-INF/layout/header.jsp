...
<body>
	<div class="main-title">
		...
		<div class="dropdown">
			...
			<div class="dropdown-menu">
				...
				<c:if test="${sessionScope.loginok==null }">
					...
					<ul>
						<li><a href="${root}/member/login">프로필</a></li>
						<li><a href="${root}/member/login">마이페이지</a></li>
					</ul>
					<ul>
						<li><a href="${root}/member/login">호스트로 전환</a></li>
					</ul>
				</c:if>
			</div>
			</c:if>
		...
		<div class="main-logo">
			<c:if test="${sessionScope.loginok==null }">
				<a href="${root }/"> <img alt="" src="../photo/logo_sm.png">
				</a>
			</c:if>
			<c:if test="${sessionScope.loginok!=null }">
				<c:if test="${sessionScope.mode=='guest'}">
		...
		<div class="main-buttons">
			<c:if test="${sessionScope.loginok==null }">
				<button type="button" class="btn btn-info" id="loginbtn"
					onclick="location.href='${root}/member/login'">Login</button>
				<button type="button" class="btn btn-default" id="signupbtn"
					onclick="location.href='${root}/member/gaip'">Sign Up</button>
			</c:if>
			<c:if test="${sessionScope.loginok!=null }">
				<c:if test="${sessionScope.mode=='guest'}">
				...
				<button type="button" class="btn btn-danger" id="logoutbtn"
					onclick="location.href='/member/kakaologout'">Logout</button>
			</c:if>
		</div>
	</div>
